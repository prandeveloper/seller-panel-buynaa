...
<<<<<<< HEAD
  ...
  async componentDidMount() {
    await Axios.get("http://35.154.86.59/api/admin/viewonerole", {
      ...
      //   if (rowData[i].emp._id === "62021240c55f460ece05e510") {
      //     this.setState({ permissions: rowData[i] });
      //     break;
      //   }
      // }
...
<<<<<<< HEAD
          ...
          key={item.id}
          onClick={(e) => {
            var hasSubscribed = localStorage.getItem("hasSubscribed");
...
<<<<<<< HEAD
        if (this.state.permissions[item.id] === true) return renderItem;
        else return null;
=======
        return renderItem
>>>>>>> main
